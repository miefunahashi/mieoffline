...
import com.mieoffline.http.fileuploadrepository.model.*;
...
public class Raw implements Producer<Consumer<HttpServletRequestResponseWrapper, ?>, Raw.RawException> {
    ...
    public Consumer<HttpServletRequestResponseWrapper, ?> apply(Void avoid) throws RawException {
       ...
       final FileUploadsReadAll fileUploadsReadAll = new FileUploadsReadAll(this.headersAvroDecoder, this.utils);
        ...
        final Producer<String, ?> getUploadsAsJsonString = new Producer<String, Exception>() {
            @Override
            public String apply(Void avoid1 ) throws Exception {
        ...
        final Function<String, FileUploadWithMultipartFormUploadRequestMissingFiles<Long>, ?> getFileUploadWithMultipartFormUploadRequestMissingFiles = new Function<String, FileUploadWithMultipartFormUploadRequestMissingFiles<Long>, Exception>() {

            @Override
            public FileUploadWithMultipartFormUploadRequestMissingFiles<Long> apply(String s) throws Exception {



                final FileUploadsReadGivenId fileUploadsReadGivenId = new FileUploadsReadGivenId(Raw.this.utils, Raw.this.sqlUtils, Raw.this.headersAvroDecoder);
                final DatabaseFunctionQuery<Long, FileUpload<Long>> something3;
                try {
                    something3 = new DatabaseFunctionQuery<>(Raw.this.iDatabase,fileUploadPartRepositoryReadSimpleFile.apply(null));
                } catch (Throwable throwable) {
                    throw new IllegalStateException("Doh", throwable);
                }
                final FileUpload<Long> fileUpload = something3.apply(Long.valueOf(s));
                ...
                final DatabaseFunctionQuery<Long, MultipartFormUploadRequestMissingFiles> objectObjectDatabaseFunctionQuery = new DatabaseFunctionQuery<>(Raw.this.iDatabase,apply);
                final MultipartFormUploadRequestMissingFiles apply1 = objectObjectDatabaseFunctionQuery.apply(fileUpload.getReferenceToFileUploadRequestMissingData().getReference());

                return new FileUploadWithMultipartFormUploadRequestMissingFiles.Builder<Long>()
                        .setFileUpload(fileUpload)
                        .setMultipartFormUploadRequestMissingFiles(apply1)
                        .build();

            }

        };
        final ResourceFileGetter resourceFileGetter = new ResourceFileGetter(getFileUploadWithMultipartFormUploadRequestMissingFiles);
